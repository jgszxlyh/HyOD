...
import fastod.*;
import dependencyDiscover.sampler.*;
import util.Gateway;
import util.Timer;
import util.Util;
...
public class FasTOD_sp {

    private final List<CanonicalOD> finishedOD = new ArrayList<>();
    ...
    // M
    private List<CanonicalOD> result = new ArrayList<>();
    // L 每层候选集
    ...
    public FasTOD_sp(long timeLimit, double errorRateThreshold) {
        this.timeLimit = timeLimit;
        this.errorRateThreshold = errorRateThreshold;
    }

    public FasTOD_sp(long timeLimit) {
        this.timeLimit = timeLimit;
    }
    ...
    private void computeODs() {
        ...
        for (AttributeSet context : contextThisLevel) {
            ...
            for (int attribute : contextIntersectCCContext) {
                ...
                for (CanonicalOD finishedod : finishedOD) {
                    if(finishedod.left == null) {
                        if (finishedod.compareTo(od) == 0) {
                            finished = true;
                            break;
                        }
                    }
                ...
                if (!finished) {
                    ...
                    for(CanonicalOD real : result){
                        if (real.left == null) {
                            if(real.compareTo(od) == 0){
                                fdcount++;
                                ccPut(context, ccGet(context).deleteAttribute(attribute));
                                // Remove all B属于R\X from Cc+(X)
                                ccGet(context);
    ...
    public static void main(String[] args) {
        ...
        FasTOD_sp f = new FasTOD_sp(1000000, -1f);

        f.initialize(sampleData);
        List<CanonicalOD> result = f.discover(sampleData);

        Set<Integer> viorows = new HashSet<>();
        List<CanonicalOD> removeod = new ArrayList<>();
        while(true) {
            CanonicalOD.clearForResult(data);
            for (CanonicalOD od : result) {
                Set<Integer> addviorows = od.validForViorows(data, -1f);
                if(!addviorows.isEmpty()){
                    removeod.add(od);
                    viorows.addAll(addviorows);
                }
            }
//            System.out.println(viorows);
            f.result.removeAll(removeod);
            if(viorows.isEmpty()){
                break;
            }
            else{
                sampleData.addRowsFromOriginalDataFrame(viorows);
//                System.out.println(sampleData.getRowsCount());
                viorows.clear();
                CanonicalOD.clearForResult(sampleData);
                f.initialize(sampleData);
