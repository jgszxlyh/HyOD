            ...
            for (AttributePair attributePair : csGet(context)) {
                ...
                if (!ccGet(context.deleteAttribute(b)).containAttribute(a)
                    ...
                    if (od.isValid(sampleData, errorRateThreshold,true)) {
                        ...
                        odcan++;
                        addvio = od.validForSample(data,errorRateThreshold,false,tupleNum);
//                        addvio = od.validForRandom(data,errorRateThreshold,false);
                        if(addvio.isEmpty()) {
                            attributePairsToRemove.add(attributePair);
                            result.add(od);
                            ocdcount++;
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("sampleTime:"+sampleTime+"ms, dicoverTime:"+(f.timer.getTimeUsed()-f.validTime)+"ms, validTime:"+f.validTime+"ms");
...
//        File csv2 = new File("sod-"+args[0].substring(0,args[0].length()-4)+"-result.txt");
//        BufferedWriter bw = new BufferedWriter(new FileWriter(csv2,true));
//        for(CanonicalOD od:result){
//            String s0 = od.toString();
//            bw.write(s0);
//            bw.newLine();
//        }
//        bw.close();
